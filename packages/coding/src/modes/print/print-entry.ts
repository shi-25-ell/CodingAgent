...
export async function runPrintEntry(
  ...
  try {
    ...
    if (report.finalAnswer) dependencies.io.stdout(`${report.finalAnswer}\n`);
    if (report.status === "completed") return { exitCode: 0, status: "completed", report };
    if (report.status === "aborted") {
      dependencies.io.stderr(`Run aborted: ${report.terminationReason}\n`);
      return { exitCode: 130, status: "aborted", report };
    }
    dependencies.io.stderr(`Run ${report.status}: ${report.terminationReason}\n`);
    return { exitCode: 1, status: report.status, report };
  } catch (error) {
